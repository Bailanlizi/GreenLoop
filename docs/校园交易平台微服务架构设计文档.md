# GreenLoop 可信交易与资金风控平台 · 架构设计文档

## 一、系统描述

### 1.1 系统概述

**GreenLoop——可信交易与资金风控平台**是一个面向个人、组织与园区场景的全栈 Web 应用，由校园二手交易场景演进而来。系统采用前后端分离架构，支持商品发布与浏览、下单、模拟支付与资金托管、退款结算、账户流水、私信沟通与后台运营，并围绕一致性、幂等性、安全性、可审计性构建交易闭环。
...
- **用户互动**：私信聊天、商品收藏、用户评价
- **信誉体系**：基于交易评价建立用户信誉评分
- **风控（规划中）**：商品敏感词、黑名单、大额/高频交易等规则引擎与三级决策（阶段三设计完成、暂缓实现）
...
| **可审计性** | 请求级 Trace 日志（traceId）已具备；后台关键操作追加式审计日志（`operation_audit_log`）列为阶段四（P1）规划，尚未实现 |
...
以下表为模拟支付与清结算所用，属于当前实现，非后续规划：

- `account`：虚拟账户表，每个用户一个账户，余额拆分为 `available_balance`（可用）与 `frozen_balance`（冻结），`version` 用于乐观锁防并发超扣。
- `account_flow`：不可变资金流水表，每笔变动记录 `availableBefore/After`、`frozenBefore/After` 余额快照与 `business_type`（如 `RECHARGE`、`PAYMENT_FREEZE`、`REFUND`、`SETTLEMENT_IN`、`SETTLEMENT_OUT`），用于全链路资金追溯。
- `recharge_order`：充值单，`request_id` + 用户唯一约束保证充值幂等。
- `payment_order`：支付单，关联订单与买家，`request_id` 保证支付幂等；状态 `PENDING → FROZEN → SETTLED / REFUNDED`。
- `account_freeze_record`：资金冻结记录，关联支付单与订单；状态 `FROZEN → RELEASED / REFUNDED`。
- `refund_order`：退款单，关联原支付单与订单。
- `settlement_order`：结算单，关联订单、支付单、买家与卖家，确认收货后把冻结资金结算给卖家。
- 资金链路：下单 → 支付冻结（`PAYMENT_FREEZE`，资金从可用转入冻结）→ 确认收货结算（`SETTLEMENT_IN/OUT`，冻结释放并转入卖家可用）→ 取消则退款（`REFUND`，冻结释放并回到买家可用）。所有金额字段为 `DECIMAL(18,2)`。
...
| Redis | 缓存中间件 | 分类、商品等热点数据可通过缓存减少数据库压力，提升读请求性能。 |
| Spring Cache | 缓存抽象 | 通过注解方式接入缓存，降低业务代码与具体缓存实现的耦合。 |
...
GreenLoop 当前采用前后端分离架构，由用户端 `campus-trade-web`、管理端 `campus-trade-admin` 和后端 API `campus-trade-api` 组成。后端以 Spring Boot 单体应用承载用户、商品、订单、配送、消息、通知、评价、文件、AI、虚拟账户与资金流水和管理端等业务模块，并通过 MySQL 保存核心业务数据，通过 Redis 提升热点数据访问性能。
