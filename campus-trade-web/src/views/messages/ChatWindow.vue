...
import { getMessageHistory, markConversationAsRead, sendMessage } from '../../api/message';
import { getUserInfo } from '../../api/user';
import { ElMessage } from 'element-plus';
import { UserFilled } from '@element-plus/icons-vue';

const props = defineProps({ otherUserId: String });
const emit = defineEmits(['new-message', 'read-state-changed']);
...
const hasUnreadIncomingMessage = (history) => {
  return history.some((msg) => (
    msg.senderId === props.otherUserId &&
    (msg.isRead === false || msg.read === false)
  ));
};

const markCurrentConversationAsRead = async (history) => {
  if (!hasUnreadIncomingMessage(history)) return;
  try {
    await markConversationAsRead(props.otherUserId);
    emit('read-state-changed');
  } catch (error) {
...
const fetchHistoryAndUserInfo = async () => {
  ...
  try {
    ...
    await markCurrentConversationAsRead(history);
    scrollToBottom();
  } catch (error) {
