<template>
    <div>
        ...
        <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="信誉分">{{ displayCreditScore }}</el-descriptions-item>
...
const displayCreditScore = computed(() => authStore.user?.creditScore ?? '--');
...
const syncCurrentUserProfile = async () => {
  ...
  try {
    ...
    authStore.setAuth({
      ...
      user: {
        ...authStore.user,
        nickname: profile.nickname ?? authStore.user?.nickname,
...
onMounted(syncCurrentUserProfile);
</script>
