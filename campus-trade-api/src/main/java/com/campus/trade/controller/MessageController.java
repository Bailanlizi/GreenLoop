...
public class MessageController {
    ...
    public Result<Map<String, Long>> getUnreadCount(@AuthenticationPrincipal AuthenticatedUser user) {
        return Result.success(Map.of("count", messageService.getUnreadCount(user.getUserId())));
    }

    @PostMapping("/read/{otherUserId}")
    public Result<Void> markConversationAsRead(@PathVariable String otherUserId, @AuthenticationPrincipal AuthenticatedUser user) {
        messageService.markConversationAsRead(user.getUserId(), otherUserId);
